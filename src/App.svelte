<script>
  ...
  $: visibleVarer = varer
                            ...
                            .filter(
                              (vare) => (
                              (vare.Varenummer+"").toLowerCase().includes(søgning.toLowerCase()) || 
                              (vare.Produktnavn+"").toLowerCase().includes(søgning.toLowerCase())
                            ));
  $: varer = varer;
  let søgning = "";
  let p = "";
...
<main>
  ...
  <div class="container">
    <Header bind:søgning bind:varer bind:slettetVarer />
            ...
            {#each visibleVarer as vare, i}
              <tr transition:fade={{ duration: 80 }} style="height: 35px;" class={i % 2 === 0 ? "odd" : "even"}>
                <td style="display: flex; justify-content: center; align-items: center; width: 100%; height: 100%;"
                  ><input style="margin: auto;" bind:checked={varer[vare.originalIndex].checked} type="checkbox" name="" id="" /></td
                >
                <td style="color: #373A86; font-weight: 3000;"><input type="text" bind:value={varer[vare.originalIndex].Varenummer} style="all: unset; width: 100%; height: 100%; margin: 0px; background:none" /></td>
                <td style="font-weight: 500; text-align:left"> <input type="text" bind:value={varer[vare.originalIndex].Produktnavn} style="all: unset; width: 100%; height: 100%; margin: 0px; background:none" /></td>
                <td style="text-align:center"><input type="text" bind:value={varer[vare.originalIndex].Pris} style="all: unset; width: 100%; height: 100%; margin: 0px; background:none" /></td>
